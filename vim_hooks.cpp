...
CUSTOM_DOC("Vim custom command for responding to a startup event")
{
    ...
    // @note Fix coming in 4coder 4.1.1
    //       active_color_table gets set on config load
    if (global_config.default_theme_name.size != 0) {
...
function void
vim_draw_cursor_mark_and_highlight(Application_Links *app, View_ID view_id, b32 is_active_view, Buffer_ID buffer_id,
...
vim_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
    ...
    vim_draw_cursor_mark_and_highlight(app, view_id, is_active_view, buffer_id,
...
vim_render_caller(Application_Links *app, Frame_Info frame_info, View_ID view_id){
    ...
    b32 is_active = (view_id == active_view_id);
#if 0
    ...
    defer {
        if (is_active)  active_color_table = temp_color_table;
    };
#endif
#if 1
    ...
    FColor margin_color = fcolor_zero();
    if (is_active) {
        margin_color = fcolor_id(defcolor_margin_active);
    }
    else {
        margin_color = fcolor_id(defcolor_margin);
    }
    Rect_f32 region = draw_background_and_margin(app, view_id, margin_color, fcolor_id(defcolor_back));
